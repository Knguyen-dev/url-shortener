...
Attributes:
- user_id: A uuid from the postgres database. By using uuid we can ensure uniqueness.
...
def create_tables():
  """Create all necessary tables for the URL shortener"""
...
def init_cassandra():
  ...
  auth_provider = PlainTextAuthProvider(
    settings.CASSANDRA_USER, settings.CASSANDRA_PASS
  )
  cluster = Cluster(
    [settings.CASSANDRA_HOST], port=settings.CASSANDRA_PORT, auth_provider=auth_provider
  )
  cassandra_session = cluster.connect(settings.CASSANDRA_KEYSPACE)
